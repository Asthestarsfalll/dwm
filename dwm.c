...
static void set_size(const Arg *arg);
...
void autostart() {
  char cmd[100];
  sprintf(cmd, "sh %s &", autostartscriptpath);
  system(cmd);
}
...
void tile(Monitor *m) {
  unsigned int i, n, mw, my, ty, mh, th;
  Client *c;
  ...
  mh = m->nmaster == 0
           ? 0
           : (m->wh - 2 * gappo - gappi * (m->nmaster - 1)) / m->nmaster;
  th = n == m->nmaster ? 0
                       : (m->wh - 2 * gappo - gappi * (n - m->nmaster - 1)) /
                             (n - m->nmaster);

  for (i = 0, my = ty = gappo, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      // h = (m->wh - ty) / (n - i);
      resize(c, m->wx + mw + gappo, m->wy + ty,
             m->ww - mw - 2 * (c->bw + gappo), th - 2 * c->bw, 0);
      ty += HEIGHT(c) + gappi;
      // if (ty + HEIGHT(c) < m->wh)
      // ty += HEIGHT(c);
    }
